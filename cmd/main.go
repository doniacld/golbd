...
func main() {
	...
	log := logger.Log{SyslogWriter: syslog, Stdout: *stdoutFlag, DebugFlag: *debugFlag, ToFilePath: *logFileFlag}
...
func shouldUpdateDNS(config *lbconfig.Config, hostname string, lg *logger.Log) bool {
...
func updateHeartbeat(config *lbconfig.Config, hostname string, lg *logger.Log) error {
	if hostname != config.Master {
		return nil
	}
...
func installSignalHandler(sighup, sigterm *bool, lg *logger.Log) {
...
func checkAliases(config *lbconfig.Config, lg logger.Log, lbclusters []lbcluster.LBCluster) {
	...
	if len(hostsToCheck) != 0 {
		...
		for _, pc := range clustersToUpdate {
			pc.WriteToLog(logger.LevelDebug, "READY TO UPDATE THE CLUSTER")
			if pc.FindBestHosts(hostsToCheck) {
				if updateDNS {
					pc.WriteToLog(logger.LevelDebug, "Should update dns is true")
					pc.RefreshDNS(config.DNSManager, config.TsigKeyPrefix, config.TsigInternalKey, config.TsigExternalKey)
				} else {
					pc.WriteToLog(logger.LevelDebug, "should_update_dns false")
				}
			} else {
				pc.WriteToLog(logger.LevelDebug, "FindBestHosts false")
			}
		}
	}
