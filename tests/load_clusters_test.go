...
func getTestCluster(name string) lbcluster.LBCluster {
	...
	return lbcluster.LBCluster{ClusterName: name,
		...
		HostMetricTable: map[string]lbcluster.Node{
...
func TestLoadClusters(t *testing.T) {
	...
	config := lbconfig.Config{Master: "lbdxyz.cern.ch",
		...
		Parameters: map[string]lbcluster.Params{"test01.cern.ch": lbcluster.Params{Behaviour: "mindless", BestHosts: 2,
			External: true, Metric: "cmsfrontier", PollingInterval: 6, Statistics: "long"},
			"test02.test.cern.ch": lbcluster.Params{Behaviour: "mindless", BestHosts: 10, External: false, Metric: "cmsfrontier", PollingInterval: 6, Statistics: "long"}}}
	...
	for _, e := range expected {
		for _, c := range lbclusters {
			if c.ClusterName == e.ClusterName {
